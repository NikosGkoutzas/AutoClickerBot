...
class ProcessEmails(ProcessEmailsInterface):
    ...
    def process_download_new_version_from_github(self, semantic_input: str) -> bool:
        ...
        try:
            ...
            os.system(
                'git clone https://github.com/NikosGkoutzas/AutoClickerBot.git')
            ...
            app_destination_dir = os.path.join(
                autoClickerBot_folder_path, 'app')
            files_destination_path = os.path.join(app_destination_dir, 'files')

            excluded_files_and_folders_list = ['__init__.py',
                                               'selenium-profile',
                                               '.env_example',
                                               '.env',
                                               '__pycache__',
            ...
            for item in os.listdir(app_destination_dir):
                if (item not in excluded_files_and_folders_list):
                    deleted_file_folder_dir = os.path.join(
                        app_destination_dir, item)

                    if (item == 'files'):
                        for inner_item in os.listdir(files_destination_path):
                            inner_path = os.path.join(
                                files_destination_path, inner_item)
                ...
                if (item not in excluded_files_and_folders_list):
                    ...
                    if (item == 'files'):
                        ...
                        for inner_item in os.listdir(temp_files_path):
                            ...
                            src_inner = os.path.join(
                                temp_files_path, inner_item)
                            dst_inner = os.path.join(
                                files_destination_path, inner_item)
            ...
            print(
                'The new version has been downloaded, installed and files have been replaced successfully.')
            os.execv(sys.executable, [sys.executable, "-m", "app.main"])

        except Exception as e:
            print(
                f'An error occured while trying to install the latest version from github: {str(e)}')
            return False
