...
class ResetFiles(ResetFilesInterface):
    ...
    def reset_all_files(self) -> None:
        ...
        '''
        try:
            self.write_files.write_number_in_file(number_of_captcha_challenges_filename,
                                                  0)
            self.write_files.write_number_in_file(number_of_github_updates_filename,
                                                  0)
            self.write_files.write_number_in_file(number_of_inserted_machines_filename,
                                                  0)
            self.write_files.write_number_in_file(number_of_removed_machines_filename,
                                                  0)
            self.write_files.write_number_in_file(total_errors_filename,
                                                  0)
            self.write_files.write_number_in_file(total_updates_filename,
                                                  0)
            self.write_files.write_number_in_file(url_current_pos_filename,
                                                  1)
            self.write_files.write_number_in_file(daily_report_sent_filename,
                                                  1)
            self.write_files.write_number_in_file(internet_errors_filename,
                                                  0)
            ...
            self.write_files.write_check_email_every_20_minutes(
                self._reset_check_email_every_20_minutes_filename())
            self.write_files.write_number_in_file(app_started_filename,
                                                  0)
            self.write_files.write_number_in_file(app_ended_filename,
                                                  0)
            self.write_files.write_number_in_file(delay_per_update_filename,
                                                  5)
            self.reset_all_updates_per_machine()

        except Exception as e:
            print(
                f'An error occured while trying to reset all files: {str(e)}')
    ...
    def reset_all_updates_per_machine(self) -> None:
        ...
        try:
            ...
            raise ValueError(
                f'File \'{updates_per_machine_filename}\' not found.')

        except ValueError:
            raise ValueError(
                f'File \'{updates_per_machine_filename}\' does not contain valid integers.')
    ...
    def _reset_check_email_every_20_minutes_filename(self) -> str:
        ...
        start_time_hour = '0' + \
            start_time_hour if (
                len(str(int(start_time_split[0]) - 1)) == 1) else start_time_hour
        start_time_min = '30'
