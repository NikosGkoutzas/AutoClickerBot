...
class ReadFiles(ReadFilesInterface):
    ...
    def read_datetime_general(self , filename: str) -> datetime.datetime | None:
        try:
            ...
            with open(filename , 'r') as f:
                return datetime.datetime.strptime(f.read().strip() , '%b %d, %Y - %H:%M:%S').time() if datetime_exists else None
    ...
    def read_app_started(self) -> bool:
        return self.general_read_int(read_app_started_filename)
    ...
    def read_daily_reset_sent(self) -> int:
