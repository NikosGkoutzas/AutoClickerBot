...
class WriteFiles(WriteFilesInterface):
    ...
    def write_app_started(self) -> None:
        number = self.read_files.read_app_started()
        self.general_write_int(read_app_started_filename , number)
    ...
    def write_app_ended(self) -> None:
        ...
        if(app_started_number != 0):
            self.write_number_in_file(read_app_started_filename , 0)
                ...
                for i in range(len(lines)): # first 2 lines are the credentials
                    if(i == 0 and 'site_username' in lines[i]):
                        if(new_username is not None):
                            f.write(f"site_username='{new_username}'\n")
                            
                    elif(i == 1 and 'site_password' in lines[i]):
                        if(new_password is not None):
                            f.write(f"site_password='{new_password}'\n")  
    ...
    def write_daily_report_sent(self) -> None:
        number = self.read_files.read_daily_reset_sent()
        self.general_write_int(daily_report_sent_filename , number)
